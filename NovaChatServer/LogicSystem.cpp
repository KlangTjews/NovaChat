...
void LogicSystem::LoginHandler(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data) {
	nlohmann::json root;
	root = nlohmann::json::parse(msg_data);

	std::cout << "user login uid is  " << root["uid"].get<int>() << " user token  is "
		<< root["token"].get<std::string>() << std::endl;
