...
class RedisConPool {
public:
	RedisConPool(size_t poolSize, const char* host, int port, const char* password) : poolSize_(poolSize), host_(host), port_(port), b_stop_(false), password_(password), counter_(0) {
		for (size_t i = 0; i < poolSize_; ++i) {
			...
			connections_.push(context); //队列推入连接
		}
		...
		check_thread_ = std::thread([this]() {
			while (!b_stop_) {
				...
				std::this_thread::sleep_for(std::chrono::seconds(1)); //隔30秒ping一次
			}
		});
	}
	...
	redisContext* getConnection() {
		...
		cond_.wait(lock, [this] {
			...
			return !connections_.empty(); //不为空则立刻返回不等待
		});
		//前为false，停止直接返回空指针
		if (b_stop_) {
			return nullptr;
